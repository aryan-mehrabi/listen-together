...
const ChatRoom = () => {
  ...
  const notAuthUserMessage = message => (
  ...
  const renderMessages = () => {
    if (!channels[selectedChannel].messages) return;
    const messages = Object.values(channels[selectedChannel].messages);
    return messages
      .sort((a, b) => a.createdAt - b.createdAt)
      .map(message => {
        if (message.from === userId) {
          return authUserMessage(message);
        } else {
          return notAuthUserMessage(message);
        }
      });
  };
  ...
  return (
    <section className="h-full flex flex-col">
      ...
      <section className="overflow-auto flex flex-col items-start flex-grow p-6">
        {
          renderMessages() /* <div className="flex items-start my-1 max-w-[80%]">
          <img
            className="w-10 mt-2"
